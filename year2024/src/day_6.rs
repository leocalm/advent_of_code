...
impl Day6 {
    ...
    fn move_guard(&self, grid: &mut Grid<char>, start_position: Point, guard: char) -> Option<(Point, char)> {
        ...
        let new_position = Point { x: start_position.x + diff.0, y: start_position.y + diff.1};
        if new_position.x < 0 || new_position.y < 0 || new_position.x > grid.rows() - 1 || new_position.y > grid.cols() - 1 {
            None
        } else {
